...
def move_and_measure(x: np.ndarray = None, y: np.ndarray = None, z: np.ndarray = None, resolution: float = None,
    ...
    GPIO_pin_config = dict(x_motor_dir=10, x_motor_step=8, y_motor_dir=18, y_motor_step=16, z1_motor_dir=13,
                           ...
                           x_cal=50, y_cal=83, z_cal=83, address_adc=0b1110110, channel_adc=0xB0, sensor_factor=2053)  # all distance units in mm

    # RPi call
    setup_RPi_GPIO(GPIO_pin_config)
    address_adc = GPIO_pin_config['address_adc'] # Making it more readable
    channel_adc = GPIO_pin_config['channel_adc'] # Making it more readable
    sensor_factor = GPIO_pin_config['sensor_factor'] # Making it more readable
    ...
    field_map = np.zeros((n_measurements, 4), dtype=float)
        ...
        for x_ind in range(x.shape[0]):
            ...
            for z_ind in range(z.shape[0]):
                ...
                for y_ind in range(y.shape[0]):
                    ...
                    sensor_value = sensor_factor * getreading(address_adc, channel_adc)
                    print("Sensor_value: " + str(sensor_value)+ " mT") 

                    # Store data
                    field_map[n, 0], field_map[n, 1], field_map[n, 2], field_map[n, 3] = x[x_ind], z[z_ind], y[
                        y_ind], sensor_value
                    n += 1
            ...
            if save_np_slice is True:  # Overwriting for now
                ...
                fname = 'Exp_' + str(exp_num) + '_' + str(var[0]) + str(var[1]) +  str(var[2]) + '.npy'
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 6:
        ...
        print("Usage: measure_magnetic_field.py x_length y_length z_length resolution experiment_number")
